...
def get_best_image_url(large, medium, small):
    """Helper to prioritize image quality."""
    ...
    try:
        ...
        top_tracks = (
            db.query(
                Track.track_id,
                Track.name,
                Track.image_url_large.label("large"),
                Track.image_url_medium.label("medium"),
                Track.image_url_small.label("small"),
                func.count(distinct(Listen.listen_id)).label("listen_count"),
                # FIX: distinct(Artist.name) ensures we don't list the artist multiple times
                # even if the track was played multiple times.
            ...
            .join(Listen, Listen.track_id == Track.track_id)
            .filter(Listen.played_at.between(start_datetime, end_datetime))
            .group_by(Track.track_id, Track.name, Track.image_url_large, Track.image_url_medium, Track.image_url_small)
        ...
        result = [
            {
                ...
                "name": track.name,
                "cover_url": get_best_image_url(track.large, track.medium, track.small),
    ...
    try:
        ...
        top_albums = (
            db.query(
                Album.album_id,
                Album.name,
                Album.image_url_large.label("large"),
                Album.image_url_medium.label("medium"),
            ...
            .filter(Listen.played_at.between(start_datetime, end_datetime))
            .group_by(Album.album_id, Album.name, Album.image_url_large, Album.image_url_medium, Album.image_url_small)
            .order_by(func.count(distinct(Listen.listen_id)).desc())
            .limit(limit)
            .all()
        )
        ...
        result = [
            {
                ...
                "name": album.name,
                "cover_url": get_best_image_url(album.large, album.medium, album.small),
